...
- **方法**：Non-parametric Bootstrap Monte Carlo Simulation（Efron, 1979）
- **核心理念**：不預測未來，但假設未來的月漲跌幅會落在歷史觀測的範圍內
- **流程**：
  1. 從標的完整歷史計算所有月漲跌幅（百分比），構成經驗分佈
  2. 每次模擬從經驗分佈中有放回隨機抽樣，產生一條未來路徑
  3. 重複 5,000 次，取 P25/P50/P75 作為悲觀/基準/樂觀三情境
- **優勢**：無分佈假設、保留真實偏態和厚尾、有漲有跌（非平滑曲線）、客觀無主觀參數
- **進階圖表**：蒙地卡羅信心區間圖（P10~P90 扇形帶 + P50 中位數線）
- **技術實作**：Web Worker 非同步計算，約 2~3 秒完成
- **分割修正**：0050 在 2025-06-18 有 1:4 分割，計算月報酬時需修正避免假性暴跌
